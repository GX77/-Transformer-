...
def train():
    ...
    if opt.resume:
        ...
        assert opt.settings == chkpoint['settings'], 'opt.settings and resume model settings are different'
        assert idx2word == chkpoint['idx2word'], 'idx2word and resume model idx2word are different'
        model.load_state_dict(chkpoint['model'])
        if chkpoint['train_mode'] == train_mode:
            optimizer.load_state_dict(chkpoint['optimizer'])
            lr = optimizer.param_groups[0]['lr']
        print("====> loaded checkpoint '{}', epoch: {}, train_mode: {}".format(opt.resume, chkpoint['epoch'], chkpoint['train_mode']))
    elif train_mode == 'rl':
    ...
    train_data = get_dataloader(opt.att_feats, opt.word_feats, captions['train'], model.pad_id,model.max_seq_len, opt.batch_size, opt.num_workers)
    val_data = get_dataloader(opt.att_feats, opt.word_feats, captions['val'], model.pad_id,model.max_seq_len, opt.batch_size, opt.num_workers, shuffle=False)
    test_captions = {}
    for fn in captions['test']:
        test_captions[fn] = [[]]
    test_data = get_dataloader(opt.att_feats, opt.word_feats, test_captions, model.pad_id,model.max_seq_len, opt.batch_size, opt.num_workers, shuffle=False)
    ...
    def forward(data, training=True):
        ...
        loss_val = 0.0
        reward_val = 0.0
        for fns, att_feats, word_feats, (caps_tensor, lengths), ground_truth in tqdm.tqdm(data, ncols=100):
            att_feats = att_feats.to(opt.device)
            word_feats = word_feats.to(opt.device)
            caps_tensor = caps_tensor.to(opt.device)

            if training and train_mode == 'rl':
                sample_captions, sample_logprobs, seq_masks = model(att_feats, word_feats, sample_max=0, mode=train_mode)
                model.eval()
                with torch.no_grad():
                    greedy_captions, _, _ = model(att_feats, word_feats, sample_max=1, mode=train_mode)
                model.train(training)
                reward = get_self_critical_reward(sample_captions, greedy_captions, fns, ground_truth, model.sos_id, model.eos_id, ciderd_scorer)
                loss = rl_criterion(sample_logprobs, seq_masks, torch.from_numpy(reward).float().to(opt.device))
                reward_val += float(np.mean(reward[:, 0]))
                # print(str(loss))
            else:
                pred = model(att_feats, word_feats, caps_tensor, lengths)
    ...
    for epoch in range(opt.max_epochs):
        ...
        if epoch in [0, 1, 2, 3, 4, 5, 28, 29, 30, 31, 32, 33, 34, 35, 39, 40, 45, 49]:
            # test
            results = []
            for fns, att_feats, word_feats,  _, _ in tqdm.tqdm(test_data, ncols=100):
                att_feats = att_feats.to(opt.device)
                word_feats = word_feats.to(opt.device)
                for i, fn in enumerate(fns):
                    att_feat = att_feats[i]
                    word_feat = word_feats[i]
                    with torch.no_grad():
                        rest, _ = model.sample(att_feat, word_feat, beam_size=opt.beam_size)
            ...
            checkpoint_path = os.path.join(checkpoint_dir, 'model_%d_%.4f.pth' % (epoch, val_loss))
